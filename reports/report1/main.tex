...
% todo: add some numeric analysis
    ...
    \item Match the potential images using RANSAC \cite{ransac}, and verify them using a probabilistic model, \textit{(if the probability is not high enough, and there isn't any image can be stitched with this image, \textbf{fail}.)}
    \item Reshape and align the images using bundle adjustment \cite{bm}, \textit{(make the stitching seamless.)}
    \item Straighten the whole panorama by silumating the centric camera and projection effect, \textit{(eliminate issues caused by different camera orientation.)}
    \item Gain compensation by averaging the pixels of overlapped areas, \textit{(remove the significant stitching edge.)}
    \item Apply multi-band blending to the final images, \textit{(remove blur effect caused by inaccurate stitching.)}
    \item Output the panorama image.
\end{enumerate}
