...
The first one is a more specific panorama stitching case - scanning image by moving camera.
...
(in the resulting, it is often a curved quadrangle, Fig \ref{fig:cml}).
...
The second extension is when the image doesn't comtain enought features.
When using the OpenCV implementation, if the image is very simple,
